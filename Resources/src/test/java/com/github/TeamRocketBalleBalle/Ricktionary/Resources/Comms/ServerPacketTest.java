...
class ServerPacketTest {
    ...
    public void legitOrderTypeTest(byte packetType, byte orderType, String name){
        Assertions.assertDoesNotThrow(() -> new ServerPacket(packetType, orderType, new Order<String>(name)));
    ...
    public void boundaryTest(byte packetType, byte orderType){
        Assertions.assertDoesNotThrow(() -> new ServerPacket(packetType, orderType, new Order<Byte>(orderType)));
    }

    @ParameterizedTest
    @CsvSource({"0,-1", "1,8", "2,9"})
    public void beyondRange(byte packet, byte orderType){
        Assertions.assertThrows(IllegalArgumentException.class, () -> new ServerPacket(packet, orderType, new Order<Byte>(packet))
        );
    }
}