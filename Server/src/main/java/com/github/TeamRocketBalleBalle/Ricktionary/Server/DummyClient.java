...
public class DummyClient {
    public void test() {
        try {
            Logger logger = LoggerFactory.getLogger("Rictionary.dummyClient");
            ...
            while (true) {
                ...
                logger.debug("Received: {}, {}" , serverPacket.getPacketType() ,serverPacket.getOrder());
                switch (serverPacket.getPacketType()) {
                    ...
                    case PacketType.LOAD_SCENE -> {
                        logger.info("Pretend im loading scene: {}", serverPacket.getOrder().getValue());
                                ...
                                public void run() {
                                    ...
                                    while (true) {
                                        try {
                                            ...
                                            ClientPacket textMessage = new ClientPacket(PacketType.GAME_INPUT, null, new Reply<>(input));
...
//
//                                        ClientPacket textMessage = new ClientPacket(PacketType.GAME_INPUT, null, new Reply<>("helloo" + System.currentTimeMillis()));
//                                        try {
//                                            Thread.sleep(5000);
//                                            out2.writeObject(textMessage);
...
//                                        }
                    ...
                    case 4 -> {
                        ...
                        ClientPacket imageLoaded = new ClientPacket(
                                PacketType.LOAD_IMG,
                                serverPacket.getOrder(),
                                new Reply<Boolean>(true)
                        );
                        out2.writeObject(imageLoaded);
                        logger.info("replied to load image: {},{},{}", imageLoaded.getPacketType(), imageLoaded.getReplyingTo(), imageLoaded.getReply());
                    }
                    case 5 -> {
                        System.out.println("send input to server: ");

                        ClientPacket gameInput = new ClientPacket(
                                PacketType.GAME_INPUT, serverPacket.getOrder(), new Reply<>(kb.nextLine())
                        );
